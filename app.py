...
st.markdown("""
<div class="hero">
    <div class="hero-eyebrow">Multi-Agent AI System · Powered by LangGraph</div>
...
with col_pipeline:
    ...
    # Map graph state keys → pipeline step keys
    step_keys = ["search_results", "scraped_content", "report", "feedback"]
    display_keys = ["search", "reader", "writer", "critic"]

    def s(step):
        if not r:
            return "waiting"
        idx = display_keys.index(step)
        state_key = step_keys[idx]
        if state_key in r and r[state_key]:
            return "done"
        if st.session_state.running:
            for i, dk in enumerate(display_keys):
                if not r.get(step_keys[i]):
                    return "running" if dk == step else "waiting"
        return "waiting"
...
if st.session_state.running and not st.session_state.done:
    ...
    with st.spinner("⚡  LangGraph pipeline is running all 4 nodes…"):
        graph = build_research_graph()
        final_state = graph.invoke({"topic": topic_val})
...
st.markdown("""
<div class="notice">
    ResearchMind · Powered by LangGraph · Built with Streamlit
</div>
""", unsafe_allow_html=True)
